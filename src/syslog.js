...
function myTimer() {
    const d = new Date();
    document.getElementById("stats").innerHTML = d.toLocaleTimeString() + " | <strong>LOCATION</strong>: Spaceport Gallima "/* + " | <strong>SCANNING</strong>: | " + rndInt*/
}
/*
async function fetchTextData() {
    try {
        const response = await fetch('nowplaying.txt');
        if (!response.ok) { // Check if the response was successful (e.g., status 200-299)
            throw new Error(`HTTP error! status: ${response.status}`);
        }
        const textData = await response.text();
        document.getElementById("stats").innerHTML += "| &#9658<strong>" + textData;
    } catch (error) {
        console.error('Error fetching text:', error);
    }
}
setInterval(fetchTextData, 1000);
*/
